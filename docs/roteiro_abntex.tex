...
\local{Brasília}
\data{2026}
\instituicao{NEXUS Research}
\tipotrabalho{Roteiro Metodológico}

\makeatletter
\hypersetup{
    pdftitle={\@title},
    pdfauthor={\@autor},
    pdfsubject={\@tipotrabalho},
    pdfkeywords={análise espectral, detecção de automação, APIs de IA, entropia espectral, séries temporais},
    colorlinks=true,
    linkcolor=blue,
    citecolor=blue,
    filecolor=magenta,
...
Roteiro para Obtenção de Parâmetros de Detecção de Automação em APIs de Inteligência Artificial: Uma Abordagem Espectral / NEXUS Project. --- Brasília, 2026.

1 v.

\textit{Roteiro Metodológico} --- NEXUS Research, 2026.

1. Análise espectral. 2. Detecção de automação. 3. APIs de IA. 4. Entropia espectral. 5. Séries temporais. I. NEXUS Project.
\end{minipage}
}
\end{center}
...
\section{Fase 1: Captura de Tráfego}

\subsection{1.1 Configuração do Ambiente}
\begin{enumerate}
...
\section{Fase 2: Extração de Features}

\subsection{2.1 Features Temporais}
...
\subsection{2.2 Features de Conteúdo}
\begin{itemize}
...
\section{Fase 3: Análise Espectral}

\subsection{3.1 Transformada de Fourier}
\begin{enumerate}
...
\subsection{3.2 Métricas Espectrais}
\begin{itemize}
...
\section{Fase 4: Formulação de Hipóteses}
...
\subsection{5.2 Análise de Sobrevivência}
...
\subsection{5.3 Significância Estatística}
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textit{Thresholds} ótimos para operação segura;
    \item Estratégia de diluição temporal baseada em síntese de ruído 1/f.
\end{enumerate}

\chapter{Considerações Éticas}
